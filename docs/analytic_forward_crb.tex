...
\textbf{suma sobre los tri\'angulos reales} de \texttt{objects/facet.obj}, cuyos
v\'ertices se colocan con el \emph{mismo} \emph{placement} del simulador escrito
como transformaci\'on \textbf{simb\'olica} de $(\rho,\varphi,h)$ (escala
...
$\sim\!10$--$15\%$ en $\sigma_\rho$. Las derivadas se obtienen
...
con error m\'aximo relativo $\sim\!1.4\times10^{-9}$. C\'odigo:
...
de $(\rho,\varphi,h)$. La malla se usa \textbf{cruda} (sin densificar): una faceta
plana son \textbf{$T=2$ tri\'angulos}.
...
$T=2$ centroides aproxima la integral de superficie que el simulador realiza sobre
$\sim\!5000$ tri\'angulos densificados (v\'ease la salvedad de la
Sec.~\ref{sec:resultados}).
\end{remark}
...
\begin{itemize}[nosep]
    ...
    rejilla del simulador (b) y la suma sobre los tri\'angulos reales (c),
    $g_{\mathrm{an}}$ reproduce la magnitud del simulador y las CRB coinciden
    dentro de $\sim\!10$--$15\%$ en $\sigma_\rho$ (Sec.~\ref{sec:resultados}). La
    \'unica salvedad residual es geom\'etrica (malla cruda de $T=2$ tri\'angulos vs
    $\sim\!5000$ densificados) y los anchos de suavizado $\tau,\kappa,\beta$; la
    comparaci\'on es ahora v\'alida \emph{tanto} en forma \emph{como} en tama\~no.
\end{itemize}
...
\midrule
0.50 &  30 & 1.00 & $9.8\times10^{-11}$ & $5.5\times10^{-10}$ & $3.9\times10^{-10}$ \\
0.50 &  90 & 1.00 & $1.2\times10^{-10}$ & $8.1\times10^{-10}$ & $4.4\times10^{-10}$ \\
0.50 & 150 & 1.00 & $1.3\times10^{-10}$ & $2.7\times10^{-10}$ & $5.8\times10^{-10}$ \\
1.00 &  30 & 1.00 & $1.7\times10^{-10}$ & $9.0\times10^{-10}$ & $4.1\times10^{-10}$ \\
1.00 &  90 & 1.00 & $2.2\times10^{-10}$ & $7.6\times10^{-10}$ & $5.9\times10^{-10}$ \\
1.00 & 150 & 1.00 & $2.2\times10^{-10}$ & $8.1\times10^{-10}$ & $4.9\times10^{-10}$ \\
1.50 &  30 & 1.00 & $2.7\times10^{-10}$ & $7.2\times10^{-10}$ & $7.0\times10^{-10}$ \\
1.50 &  90 & 1.00 & $2.3\times10^{-10}$ & $9.2\times10^{-10}$ & $9.3\times10^{-10}$ \\
1.50 & 150 & 1.00 & $4.2\times10^{-10}$ & $1.4\times10^{-9}$  & $8.6\times10^{-10}$ \\
\bottomrule
\end{tabular}
\end{center}
\begin{equation*}
    \textbf{Error m\'aximo relativo (anal\'itico vs FD central):}\quad
    1.4\times10^{-9}\ \ \ll\ \ 10^{-6}\ \ \Rightarrow\ \ \textbf{PASS}.
\end{equation*}
Como CRB de referencia, en $(\rho,\varphi,h)=(1,90^\circ,1)$ se obtiene
$\sigma_\rho\approx0.0050$\,m, $\sigma_\varphi\approx1.55^\circ$,
$\sigma_h\approx0.013$\,m --- valores en \textbf{magnitud f\'isica}, del mismo
...
\begin{remark}[Escalas ahora comparables]
Con la variante (a)+(b)+(c) las \textbf{magnitudes absolutas} del CRB anal\'itico y
del FD \textbf{s\'i} son directamente comparables: comparten amplitud f\'isica,
rejilla del simulador y la malla real de la faceta. La superposici\'on a
\emph{magnitud real} (Fig.~\ref{fig:avf-abs}) muestra las elipses anal\'iticas
(verde) cayendo sobre las FD (rojo) en toda la rejilla, con
$\sigma_\rho^{\mathrm{an}}/\sigma_\rho^{\mathrm{FD}}\in[0.86,1.05]$ y
$\sigma_\varphi^{\mathrm{an}}/\sigma_\varphi^{\mathrm{FD}}\in[0.6,1.35]$
(Tabla~\ref{tab:avf}). Se conserva adem\'as la superposici\'on \emph{normalizada en
forma} (igualando la $\sigma_\rho$ mediana) como verificaci\'on de
orientaci\'on/aspecto: al ser ya comparables en tama\~no, ambas curvas quedan casi
indistinguibles.
\end{remark}
...
rejilla ($\sigma_\rho$ anal./FD $\in[0.86,1.05]$), lo que antes era imposible con
la faceta puntual. Archivo \texttt{plots/crb\_regions\_analytic\_vs\_fd.png}.}
\label{fig:avf-abs}
\end{figure}
...
de \textbf{magnitud}.
\begin{enumerate}[nosep]
    \item \textbf{Magnitud comparable.} La amplitud f\'isica, la rejilla del
    simulador y la suma sobre los tri\'angulos reales hacen que
    $\sigma_\rho^{\mathrm{an}}\approx\sigma_\rho^{\mathrm{FD}}$ (raz\'on
    $0.86$--$1.05$) en toda la rejilla; en el l\'imite duro
    ...
    coincide ($\approx8.4$ anal.\ vs $8.2$ FD) y la orientaci\'on es consistente.
    \item \textbf{Salvedad residual.} La malla se usa cruda ($T=2$ tri\'angulos)
    mientras el simulador densifica a $\sim\!5000$; la suma sobre $2$ centroides es
    una cuadratura gruesa de la integral de superficie, suficiente para acercar la
    magnitud pero no id\'entica. Persisten los anchos $\beta,\kappa,\tau$.
\end{enumerate}
\textbf{Veredicto:} de todas las variantes, la (a)+(b)+(c) es la \textbf{m\'as
fiel}: derivadas exactas, Fisher bien condicionada \textbf{y} magnitud comparable
al rasterizador. El FD sigue siendo valioso como verificaci\'on independiente y
...
\texttt{facet.obj} con \emph{placement} simb\'olico. Resultado: el CRB anal\'itico
es \textbf{comparable en magnitud} con el FD ($\sigma_\rho$ anal./FD $0.86$--$1.05$
en toda la rejilla) y la superposici\'on a magnitud real coincide sin normalizar.
Salvedades: la malla se usa \textbf{cruda} ($T=2$ tri\'angulos vs $\sim\!5000$
densificados en el simulador), de modo que la suma es una \textbf{cuadratura
geom\'etrica gruesa} de la integral de superficie; la convenci\'on de altura es la
del simulador (faceta de $z=0$ a $z=h$ tras la elevaci\'on $z_{\min}\!\to\!0$); la
ventana de bins se congela alrededor del pulso (\texttt{freeze\_bins}); se asume
$\nabla d_i\neq\vect0$ (Hip.~\ref{as:geom}) e $\matr{I}\succ0$; y los par\'ametros
$(\beta,\kappa,\tau)$ recuperan el l\'imite duro cuando $\beta,\kappa\to\infty$,
$\tau\to0$.
