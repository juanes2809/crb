...
ruta FD quedan en la \textbf{misma escala} y son comparables en magnitud
(convergen al bajar el suavizado), a diferencia de la versi\'on previa con
ganancia y rejilla propias.
\end{abstract}
...
reproduce la \emph{magnitud} del rasterizador a primer orden --- la amplitud
escala con $h$ como lo hace la malla --- de modo que el CRB anal\'itico y el FD
quedan en la \textbf{misma escala f\'isica} (Sec.~\ref{sec:resultados}). El
residual proviene de la faceta puntual (\'area \'unica en el centroide $z=h/2$)
frente al render de malla y del suavizado finito; al bajar $\tau,1/\beta,1/\kappa$
la CRB anal\'itica converge hacia la FD.
\end{remark}
...
\begin{itemize}[nosep]
    ...
    Prop.~\ref{prop:fd}).
    \item \textbf{Salvedades (honestas):} con la variante (a)+(b) la
    \emph{magnitud} del CRB \textbf{s\'i} es comparable con la del simulador ---
    misma amplitud f\'isica ($I_{\mathrm{laser}}\!\cdot\!w h$, Obs.~\ref{rem:area})
    y misma rejilla $64\times64$/\texttt{bin} real ---, de modo que ambas elipses
    quedan en la \textbf{misma escala} (Sec.~\ref{sec:resultados}). Quedan como
    idealizaciones la \textbf{faceta puntual} (\'area \'unica $w h$ en el centroide
    $z=h/2$, en vez del render de malla) y los par\'ametros de suavizado
    $\tau,\kappa,\beta$; por eso el ajuste no es exacto sino \emph{convergente}
    (al bajar el suavizado). La comparaci\'on con la ruta FD ahora abarca tanto la
    \textbf{magnitud} como la \textbf{forma/orientaci\'on} de las elipses.
\end{itemize}
...
de \'el. Que ahora coincidan tanto la \emph{magnitud} como la \emph{forma} de las
elipses entre ambas rutas (Sec.~\ref{sec:resultados}) es justamente lo que
\textbf{valida} la f\'isica y la geometr\'ia del modelo anal\'itico: dos
derivaciones distintas del mismo fen\'omeno llegan a la misma estructura de
incertidumbre y a la misma escala, una con derivadas exactas y otra con el render
de malla completo.
...
suave del simulador (dos rebotes, cuatro cosenos, $4\pi^2$). Con la variante
(a)+(b) el modelo anal\'itico de faceta puntual usa adem\'as la \emph{misma}
amplitud f\'isica ($I_{\mathrm{laser}}\!\cdot\!w h$, Obs.~\ref{rem:area}) y la
\emph{misma} rejilla $64\times64$/\texttt{bin} real, de modo que su CRB queda en
la \textbf{misma escala} que el rasterizador (el residual restante es la
idealizaci\'on de faceta puntual y el suavizado finito). Lo que compartimos es la
\emph{f\'isica}, la \emph{rejilla} y la \emph{estructura} Fisher--Poisson.
\end{remark}
...
\begin{remark}[Escalas ahora comparables (variante (a)+(b))]
...
simulador, las \textbf{magnitudes absolutas} del CRB anal\'itico y del FD
\textbf{s\'i} son comparables: caen en el mismo orden y las elipses $3\sigma$
casi se superponen a \emph{magnitud real} (Fig.~\ref{fig:avf-abs}). Sobre el grid
\emph{fixed\_h}, el anal\'itico da $\sim0.5$--$0.7\times$ la $\sigma$ del FD (p.\,ej.\
en $(1,90^\circ)$: $\sigma_\rho^{\mathrm{an}}=2.3\times10^{-3}$ vs
$\sigma_\rho^{\mathrm{FD}}=4.2\times10^{-3}$\,m; $\sigma_\varphi^{\mathrm{an}}=1.1^\circ$
vs $\sigma_\varphi^{\mathrm{FD}}=1.4^\circ$), un factor peque\~no atribuible a la
faceta puntual y al suavizado finito. Conservamos adem\'as la superposici\'on
\emph{normalizada en forma} (Fig.~\ref{fig:avf-shape}) como chequeo de
orientaci\'on y aspecto.
\end{remark}
...
\emph{magnitud real}: con la variante (a)+(b) las elipses $3\sigma$ quedan en la
\textbf{misma escala} y casi se superponen en todo el grid $(\rho,\varphi)$.
...
Ahora que las escalas coinciden, ``mejor'' combina \textbf{magnitud comparable},
\textbf{calidad/suavidad de la derivada} y \textbf{sensatez de la forma}:
\begin{enumerate}[nosep]
    \item \textbf{Magnitud.} Con la variante (a)+(b) el CRB anal\'itico y el FD
    est\'an en la \textbf{misma escala f\'isica} (mismas unidades y
    normalizaci\'on $I_{\mathrm{laser}}\!\cdot\!w h$, misma rejilla): el anal\'itico
    reproduce $\sim0.5$--$0.7\times$ la $\sigma$ del FD y converge hacia \'el al
    bajar el suavizado (Fig.~\ref{fig:avf-abs}).
    ...
    \emph{no} reproduce exactamente el rasterizador de malla; el peque\~no residual
    y la dependencia de $\beta,\kappa,\tau$ son el precio de la diferenciabilidad.
\end{enumerate}
\textbf{Veredicto:} para un CRB \emph{diferenciable}, la elipse \textbf{anal\'itica}
es ahora comparable en \textbf{magnitud} con la FD y, por sus derivadas exactas,
mejor condicionada; el FD sigue siendo valioso como verificaci\'on independiente y
como modelo de malla completo.
...
\begin{itemize}[nosep]
    ...
    \item con \textbf{amplitud f\'isica} ($I_{\mathrm{laser}}\!\cdot\!w h$) y la
          ...
          (a)+(b)), el CRB anal\'itico y el FD quedan en la \textbf{misma escala}
          y son comparables en magnitud (el anal\'itico $\sim0.5$--$0.7\times$ el
          FD, convergiendo al bajar el suavizado).
\end{itemize}
Salvedades: el modelo sigue siendo una \textbf{faceta puntual} con \'area \'unica
$A_{\mathrm{eff}}=w h$ colocada en el centroide $z=h/2$ (no el render de malla,
Obs.~\ref{rem:area}); la geometr\'ia (rejilla del simulador) es fija; se asume
