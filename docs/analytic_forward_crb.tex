...
semi-anchura $\sim1/\kappa$ (metros). Por defecto $\kappa=60$ es una elecci\'on
\textbf{gen\'erica de esta rama base} (penumbra $\sim1.7$\,cm), un valor
intermedio entre las dos escalas de p\'ixel en juego --- el de la rejilla propia
de esta rama ($6.25$\,cm) y el del simulador ($0.39$\,cm) --- y por tanto
\emph{no} atado a ninguna de ellas. En la rama a+b+c
(\texttt{cursor/analytic-forward-mesh-sum-9b29}) la penumbra se ata al p\'ixel del
simulador, $\kappa=1/\text{pixel\_pitch}=256$.
...
\begin{remark}[La extrapolaci\'on de Richardson es inapropiada sobre un forward escalonado]
La variante de \emph{Richardson} que ofrece \texttt{crb\_polar\_functions.py}
(refinar $\delta$ y extrapolar el error a orden superior) presupone una
\textbf{expansi\'on de error suave} en potencias de $\delta$ del tipo
$D_\delta f = f' + c_2\delta^2 + c_4\delta^4 + \dots$. Esa hip\'otesis
\textbf{no se cumple} sobre el forward duro: por la Prop.~\ref{prop:fd}, cerca de
un salto $D_\delta f\propto1/\delta$ (crece, no decrece, al refinar) y sobre las
mesetas es id\'enticamente $0$, de modo que no existe la serie que Richardson
extrapola. Aplicarla amplificar\'ia el artefacto $1/\delta$ en lugar de
cancelarlo. Por eso \textbf{no} se usa en el pipeline (siempre se emplea el
esquema \texttt{central}); se se\~nala aqu\'i solo como advertencia. El forward
\emph{suave} de este trabajo evita el problema de ra\'iz: su derivada es
anal\'itica y exacta, sin dependencia de $\delta$.
\end{remark}
...
\begin{itemize}[nosep]
    ...
    \textbf{standalone}, con su \emph{propia} rejilla fija de p\'ixeles de piso
    ($8\times8$) y una ventana de bins alrededor del pulso. \textbf{No} usa el
    cubo del simulador ni mediciones simuladas.
\end{itemize}
...
no para procesar un array de mediciones. Tampoco compartimos el mismo array de
salida: el anal\'itico tiene su propia rejilla, m\'as peque\~na ($8\times8$
p\'ixeles $\times$ bins del pulso, frente a $64\times64\times T$). El punto de
contacto entre ambos es la \emph{f\'isica} que modelan y la \emph{estructura}
Poisson--Fisher, no un intercambio de datos.
\end{remark}
...
\begin{itemize}[nosep]
    ...
    Prop.~\ref{prop:fd}).
    \item \textbf{Salvedades (honestas):} la \emph{magnitud} absoluta del CRB
    depende de idealizaciones --- el \'area efectiva constante doblada en la
    ganancia $G$ (Obs.~\ref{rem:area}), la rejilla propia $8\times8$ (m\'as
    peque\~na), y los par\'ametros de suavizado $\tau,\kappa,\beta$ ---, de modo que
    $g_{\mathrm{an}}$ \textbf{no} reproduce 1:1 la magnitud del simulador. Por eso
    la comparaci\'on v\'alida con la ruta FD es de \textbf{forma/orientaci\'on} de
    las elipses (que s\'i coincide), no de tama\~no (Sec.~\ref{sec:resultados}).
\end{itemize}
...
de \'el. Que la \emph{forma} de las elipses coincida entre ambas rutas
(Sec.~\ref{sec:resultados}) es justamente lo que \textbf{valida} la geometr\'ia del
modelo anal\'itico: dos derivaciones distintas del mismo fen\'omeno llegan a la
misma estructura de incertidumbre, una con derivadas exactas y otra con el
render de malla completo.
