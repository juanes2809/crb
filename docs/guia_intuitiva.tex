...
\begin{description}
\item[\textbf{PR\#1 --- Plots polares (ruta FD, tarea original).}] La base. Corre
  el \textbf{simulador rasterizado} en una rejilla de poses $(\rho,\varphi)$,
  calcula el CRB por \textbf{diferencias finitas} y dibuja las burbujas $3\sigma$.
  Es el punto de partida y sirve como \textbf{verdad de referencia} contra la que
  se compara todo lo dem\'as. No hay derivadas exactas: todo es num\'erico.
...
\item[\textbf{PR\#3 --- Caso (a)+(b): amplitud f\'isica + rejilla del simulador.}]
  ...
  $\sigma_\rho$ sale \textbf{$\sim0.5$--$0.7\times$} el del FD (subestima la
  incertidumbre).
...
\emph{minúsculo} (demasiado optimista, $\sigma_\rho\sim0.5$--$0.7\times$ FD).
...
integral de superficie (los cambios de $128\to512\to2048$ tri\'angulos son
$<0.5\%$). Pero adem\'as la resoluci\'on angular la fija el \textbf{borde} de la
...
\emph{propias} del simulador. El resultado (medido) es que $\sigma_\rho$ y
$\sigma_\varphi$ quedan ambas en \textbf{raz\'on $1.00$--$1.12$} respecto al FD en
\textbf{toda} la rejilla.
...
rejilla, en \textbf{ambos} ejes ($\sigma_\rho$ y $\sigma_\varphi$, razones
$1.00$--$1.12$). Que caigan una sobre otra \emph{sin} normalizar es la prueba de
que las dos rutas concuerdan en forma \emph{y} en tama\~no.}
...
\midrule
\textbf{PR\#1}\newline Plots FD &
Simulador rasterizado + CRB por diferencias finitas; burbujas $3\sigma$. &
...
Misma \textbf{escala}; pero $\sigma_\rho\sim0.5$--$0.7\times$ (rango demasiado
afilado). &
...
\textbf{Ambos} $\approx1{:}1$: $\sigma_\rho$ y $\sigma_\varphi$ en razón
$1.00$--$1.12$ en toda la rejilla. &
