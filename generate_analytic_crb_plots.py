...
def _analytic_config_key():
    """Fingerprint of the ForwardConfig the cached grid was computed with.

    Combines ``ForwardConfig().as_key()`` (every structural field of the closed
    form) with an explicit branch tag, so a cache produced by a different config
    (e.g. an older pre-densification build) is detected and recomputed instead of
    silently reused.
    """
    return ("pr4-abc", ForwardConfig().as_key())
...
def build_or_load_analytic() -> Dict[str, List[Dict[str, Any]]]:
    ...
    if ANALYTIC_CACHE.exists():
        ...
        if "fixed_h" in cache and "with_h" in cache and have_key == want_key:
            print(f"Reusing analytic cache {ANALYTIC_CACHE.name} (config_key matches)")
            return cache
        if have_key != want_key:
            print(
                f"Analytic cache config_key mismatch -> recomputing.\n"
                f"  cached: {have_key}\n  wanted: {want_key}"
            )
...
def write_latex_table(
    ...
    for ar in a_grid:
        ...
        if fr is not None:
            ...
            f_as = f"{fm['aspect']:.1f}"
            if fr["sigma_rho"] > 0:
                sr_ratios.append(ar["sigma_rho"] / fr["sigma_rho"])
        else:
            f_sr = f_sp = f_as = "--"
        rows.append(
    ...
    body = "\n".join(rows)
    if sr_ratios:
        ratio_str = f"${min(sr_ratios):.2f}$--${max(sr_ratios):.2f}$"
    else:
        ratio_str = "n/d"
    frag = (
        ...
        "del simulador y la suma sobre los tri\\'angulos reales densificados, las magnitudes "
        "absolutas ya son comparables (raz\\'on $\\sigma_\\rho$ anal./FD " + ratio_str +
        " en este grid).}\n"
        "\\label{tab:avf}\n"
